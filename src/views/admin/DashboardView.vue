<template>
  <div class="font-work-sans">
    ...
    <div class="p-container-padding flex flex-col gap-stack-lg">
      ...
      <!-- ── Job detail overlay + slide panel ── -->
            ...
            >
              ...
              <div class="flex-grow overflow-y-auto p-stack-lg space-y-stack-lg">
                ...
                <section class="space-y-2">
                  ...
                  <!-- Mark as paid button for pay_on_pickup -->
                  <button
...
function deliveryBadge(type) {
  ...
  }[type] ?? { cls: 'bg-gray-100 text-gray-800', label: type }
}
...
function statusLabel(status) {
  return { pending: 'Pending', printing: 'Printing', ready: 'Ready', delivered: 'Delivered' }[status] ?? status
}
...
function alertStyle(type) {
  ...
  }[type] ?? { container: 'bg-gray-50 border-gray-300', icon: 'text-gray-500', symbol: 'info', title: 'text-on-surface', sub: 'text-on-surface-variant' }
}
...
// ── Panel pill styling ────────────────────────────────────────────────────────
const statusOrder = ['pending', 'printing', 'ready', 'delivered']

function pillClass(val, i) {
  const currentIdx = statusOrder.indexOf(admin.selectedJob?.status ?? 'pending')
  if (i < currentIdx)  return 'bg-outline-variant text-on-surface-variant'                             // past
  if (i === currentIdx) return 'bg-secondary-container text-white shadow-md scale-105'                 // current
  return 'border-2 border-outline-variant text-on-surface-variant hover:border-primary hover:text-primary' // future
}
...
// Next logical status for the footer "Update order status" button
const nextStatus = computed(() => {
  const order = statusOrder
  const idx = order.indexOf(admin.selectedJob?.status ?? 'pending')
