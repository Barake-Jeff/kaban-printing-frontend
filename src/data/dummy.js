// ─── DUMMY DATA ──────────────────────────────────────────────────────────────
// All data here mirrors the shape of what the NestJS API will return.
// When the API is ready, replace these with real fetch/axios calls in
// src/composables/useApi.js — no component changes needed.
// ─────────────────────────────────────────────────────────────────────────────
...
export const JOBS = [
  {
    ...
    paymentMethod: 'mpesa',  // 'mpesa' | 'pay_on_pickup'
    paymentStatus: 'paid',   // 'paid' | 'unpaid' | 'pay_on_pickup'
    mpesaRef: 'QHX3K8P2',
    status: 'ready',         // 'pending' | 'printing' | 'ready' | 'delivered'
    cost: 80,
    deliveryFee: 0,
...
// All jobs in the admin queue (all users)
...
export const PRICING = {
  bwPerPage:      5,
  colorPerPage:   20,
  doubleSidedMultiplier: 1.8, // slightly less than 2x — 2 sides, 1 sheet
  deliveryFee:    50,
}

// ─── HELPERS ─────────────────────────────────────────────────────────────────
export function calculateCost({ pages, copies, colorMode, sides, deliveryType }) {
...
export function getStatusLabel(status) {
  const map = { pending: 'Pending', printing: 'Printing', ready: 'Ready', delivered: 'Delivered' }
  return map[status] ?? status
}

export function getPaymentLabel(method) {
  const map = { mpesa: 'M-Pesa', pay_on_pickup: 'Pay on pickup' }
